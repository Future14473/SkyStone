...
public class compute {
	...
	static Mat threshold(Mat input, double[] colRange, double[] satRange, double[] valRange) {
		Mat output = MatAllocator.getMat("output");
	...
	static void findContours(Mat input, boolean externalOnly, List<MatOfPoint> contours) {
		Mat hierarchy = MatAllocator.getMat("hierarchy");
		contours.clear();
		int mode;
		if (externalOnly) {
	...
	static void floodFill(Mat canvas, Point origin) {
		...
		Imgproc.floodFill(canvas, MatAllocator.getMat("anon1", canvas.height() + 2, canvas.width() + 2, canvas.type())
				, origin,
				new Scalar(0, 0, 0));
	}
	...
	static Mat distanceTransform(Mat inp, int threshold) {
		Mat proc = MatAllocator.getMat("proc");
		/*
	...
	static Mat fillHoro(Mat inp) {
		Mat proc = MatAllocator.getMat("proc1");
		/*
		 * Imgproc.distanceTransform(inp,proc, Imgproc.DIST_C,5);
		 *
		 * Core.inRange(proc,new Scalar(threshold), new Scalar(255), proc);
	...
	static Mat combine(Mat one, Mat two) {
		Mat ret = MatAllocator.getMat("ret");
	...
	static Mat subtract(Mat one, Mat two) {
		Mat ret = MatAllocator.getMat("ret2");
	...
	static Mat add(Mat one, Mat two) {
		Mat ret = MatAllocator.getMat("ret3");
	...
	static Mat flip(Mat one) {
		Mat ret = MatAllocator.getMat("ret5");
...
//	}
	...
	static Mat enforceChannels(Mat inp, int channel) {
		Mat out = MatAllocator.getMat("out6");
		
		List<Mat> channels = new ArrayList<Mat>();
		Core.split(inp, channels);
		
		for (int i = channel; i < channels.size(); i = i) {
			channels.remove(i);
		}
	...
	static void clockwise(Mat in) {
		if (in.width() < in.height()) {
			Core.rotate(in, in, Core.ROTATE_90_CLOCKWISE);
		}
	}
	
	static void counterCLockwise(Mat in) {
		if (in.width() > in.height()) {
			Core.rotate(in, in, Core.ROTATE_90_COUNTERCLOCKWISE);
		}
	}
	...
	public static double getHistogramfast(Mat in) {
		...
		Mat[] histData = new Mat[]{
				MatAllocator.getMat("anon7"),
				MatAllocator.getMat("anon8"),
				MatAllocator.getMat("anon 9")
		};
		
		Mat output = MatAllocator.getMat("output21", 512, 512, in.type());
	...
	static void getHistdataf(Mat in, int channel, Mat out, int histsize) {
		...
		Imgproc.calcHist(Arrays.asList(in), new MatOfInt(channel), // color channel
				MatAllocator.getMat("anon69"), out, new MatOfInt(histsize), // size
